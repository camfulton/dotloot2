...
class PoeNinjaClient:
    def __init__(self, league):
        ...
        self.ITEM_TYPES = {
            ...
            "unique accessories": OverviewAndType(
                self.ITEM_OVERVIEW, "UniqueAccessory"
            ),
            "uniques": OverviewAndType(self.ITEM_OVERVIEW, "ALL_UNIQUES"),
            "exclusive uniques": OverviewAndType(
                self.ITEM_OVERVIEW, "EXCLUSIVE_UNIQUES"
            ),
        }
        self.GROUP_BY_TYPES = {
            "STR_GROUPS": {
        ...
        self.ALL_UNIQUE_TYPES = [
            ...
            if "unique" in k and k not in ["uniques", "exclusive uniques"]
        ]
        self.qualifiers = {}

    def get_item_groups_for_block(self, block):
        print(
            f"  $ Looking up price data for: {block.lines[0].block_name} on poe.ninja"
        )
        ...
        item_data, low_value_bases = self.fetch_item_data_and_low_value_bases(
            lookup_data
        )
    ...
    def group_by(self, item_groups, group_by):
        ...
        for group in item_groups:
            ...
            sorted_group = sorted(group, key=get_group_by)  # type: ignore
        ...
        for item_group in item_groups:
            for item in item_group:
                ...
                if (
                    variant
                    and variant.upper()
                    not in constants.VARIANTS_THAT_MATTER_FOR_FILTERS
                ):
    ...
    def get_matching_items(self, data, lookup_data):
        matching_items = [
            item
            for item in data
            if not self.low_confidence(item)
            and self.in_price_range(item, lookup_data.min_chaos, lookup_data.max_chaos)
            and self.item_passes_block_qualifiers(item)
            and not items.nodrop(item)
            and not self.is_type(item, ["RELIC", "SUPPORTER_RELIC"])
            and not self.linked(item)
        ]
    ...
    def is_type(self, item, item_types):
        for item_type in item_types:
            if (
                item.get(self.PROPERTY_SYNTAX["frame_type"])
                == constants.FRAME_TYPES[item_type]
            ):
                return True
    ...
    def get_low_value_bases(self, data, lookup_data):
        low_value_bases = [
            item.get("baseType", "ITEM_HAS_NO_BASE")
            for item in data
            if not self.low_confidence(item)
            and not self.in_price_range(
                item, lookup_data.min_chaos, lookup_data.max_chaos
            )
            and self.item_passes_block_qualifiers(item)
            and not items.nodrop(item)
            and not self.is_type(item, ["RELIC", "SUPPORTER_RELIC"])
    ...
    def linked(self, item):
        ...
        if (
            item.get("links", 0) > 0
            and item.get("name", "").lower() not in constants.ONLY_LINKED
        ):
            return True
    ...
    def parse_lookup(self, block):
        ...
        item_type, min_chaos, max_chaos = pattern.match(block.lookup).groups()  # type: ignore
    ...
    def clear_then_set_qualifiers(self, block):
        ...
        for line in block.lines:
            ...
            if qualifier:
                qualifiers[qualifier] = line.value
